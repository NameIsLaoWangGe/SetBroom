(function () {
    ...
    (function (lwg) {
        ...
        (function (Global) {
            ...
            function _levelInformation() {
                ...
                if (lwg.Global._gameLevel > 20) {
                    index = lwg.Global._gameLevel - Math.floor(Math.random() * 20);
                }
                else {
        ...
        (function (Animation) {
            ...
            function HintAni_01(node, upNum, time1, stopTime, downNum, time2, func) {
                node.alpha = 0;
                Laya.Tween.to(node, { alpha: 1, y: node.y - upNum }, time1, null, Laya.Handler.create(this, function () {
                    Laya.Tween.to(node, { y: node.y - 15 }, stopTime, null, Laya.Handler.create(this, function () {
                        Laya.Tween.to(node, { alpha: 0, y: node.y + upNum + downNum }, time2, null, Laya.Handler.create(this, function () {
                            if (func !== null) {
                                func();
                            }
                        }), 0);
    ...
    class UIMain_Besom extends Laya.Script3D {
        ...
        move_Up() {
            ...
            this.upAccelerated -= 0.001;
        }
        move_Down() {
            this.downAccelerated = 0.012;
            this.self.transform.localPositionY -= this.downAccelerated;
            this.downAccelerated += 0.001;
        }
        move_Left() {
            this.targetHand = this.selfScene['UIMain'].targetHand;
            let position = this.targetHand.transform.position;
            this.self.transform.position = position;
            this.self.transform.localRotationEulerZ += this.rotateAccLeft;
            this.rotateAccLeft += 0.1;
        }
        move_Right() {
            this.targetHand = this.selfScene['UIMain'].targetHand;
            let position = this.targetHand.transform.position;
            this.self.transform.position = position;
            this.self.transform.localRotationEulerZ += this.rotateAccRight;
            this.rotateAccRight += 0.1;
        }
        move_Right_Up() {
        }
        move_Right_Down() {
        ...
        rotate_leftAndRight() {
            if (lwg.Global._besomRotate === lwg.Enum.BesomRotateType.right) {
                this.self.transform.localRotationEulerZ -= 0.5;
                this.timeAddControl();
            }
                ...
                let rotae = this.self.transform.localRotationEulerZ;
                if (rotae > 0) {
                    this.rotateAcc += 0.01;
                }
                else if (rotae <= 0) {
                ...
                this.self.transform.localRotationEulerZ += this.rotateAcc;
            }
        }
        timeAddControl() {
            this.timeAdd++;
    ...
    class Hand extends Laya.Script3D {
        ...
        onUpdate() {
            ...
            if (Math.abs(diffX) < 0.08 && Math.abs(diffY) < 0.008) {
    ...
    class UIMain_Gold extends Laya.Script3D {
        ...
        onUpdate() {
            ...
            if (Math.abs(diffX) < 0.13 && Math.abs(diffY) < 0.13) {
    ...
    class UIMain extends Laya.Script3D {
        ...
        onMouseMove() {
            ...
            if (moveY > 0.02 && lwg.Global._besomMove === lwg.Enum.BesomMoveType.connectStatic) {
                console.log('颠起！');
                this.Besom.transform.position.y += 0.5;
                lwg.Global._besomMove = lwg.Enum.BesomMoveType.up;
                this.Besom['UIMain_Besom'].upAccelerated = 0.035;
            }
            ...
            if (level === 3 || (level + 1) % 4 === 0) {
                for (let i = 0; i < 4; i++) {
                    for (let j = 0; j < 10; j++) {
                        ...
                        gold.transform.localPositionX = this.goldTem.transform.localPositionX - j * 0.063;
                        gold.transform.localPositionY = this.goldTem.transform.localPositionY - i * 0.063;
    ...
    class UIPifu extends Laya.Script {
        ...
        createPifuList() {
            ...
            this.PifuList.scrollTo(this.listFirstIndex);
        ...
        btnBuyClickUP(event) {
            ...
            if (lwg.Global._goldNum < price || lwg.Global._notHavePifuSubChaoren.length <= 0) {
                ...
                let pifuIndex = lwg.Enum.PifuAllName[lwg.Global._notHavePifuSubChaoren[random]];
                this.listFirstIndex = pifuIndex;
                console.log('购买了第' + pifuIndex + '位置的皮肤');
                lwg.Global._havePifu.push(lwg.Enum.PifuAllName[pifuIndex]);
                this.noHaveSubChaoren();
                this.PifuList.tweenTo(pifuIndex, 500, Laya.Handler.create(this, function () {
                    this.buyCompelet();
                }));
            }
        }
